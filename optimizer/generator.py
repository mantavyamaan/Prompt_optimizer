...
def thompson_pick(category: str) -> str:
    with conn() as connection:
        rows = connection.execute("SELECT strategy,SUM(gate_outcome='promoted') wins,COUNT(*) total FROM mutation_log WHERE category=? GROUP BY strategy", (category,)).fetchall()
    history = {row["strategy"]: (row["wins"] or 0, row["total"]) for row in rows}
    return max(STRATEGIES, key=lambda strategy: random.betavariate(1 + history.get(strategy, (0, 0))[0], 1 + history.get(strategy, (0, 0))[1] - history.get(strategy, (0, 0))[0]))


def _examples(exemplars: list[dict]) -> str:
    return "\n\n".join(f"Input: {json.dumps(item['input'], sort_keys=True)}\nOutput: {json.dumps(item['expected'], sort_keys=True)}" for item in exemplars[:2])
...
def find_best_few_shot_examples(category: str, prompt_id: str, limit: int = 2) -> list[dict]:
    with conn() as connection:
        rows = connection.execute("""
            ...
            WHERE b.category = ? AND e.prompt_id = ? AND b.difficulty IN ('hard', 'adversarial')
            ORDER BY RANDOM() LIMIT 50
        """, (category, prompt_id)).fetchall()
    candidates = []
    for row in rows:
        try:
...
def _mutate(strategy: str, modules: PromptModules, parent_id: str, category: str) -> tuple[str, PromptModules]:
    ...
    if strategy == "crossover":
        with conn() as connection:
            row = connection.execute("SELECT modules FROM prompts WHERE category=? AND prompt_id != ? AND status IN ('champion', 'candidate') ORDER BY RANDOM() LIMIT 1", (category, parent_id)).fetchone()
        if row:
...
def _persist(prompt: Prompt) -> None:
    with conn() as connection:
        connection.execute("""INSERT INTO prompts(prompt_id,category,modules,gen_params,parent_id,lineage_depth,mutation_note,compiled_hash,status)
            VALUES(?,?,?,?,?,?,?,?,?)""", (prompt.prompt_id, prompt.category, dumps(prompt.modules.model_dump()), dumps(prompt.gen_params.model_dump()),
             prompt.parent_id, prompt.lineage_depth, prompt.mutation_note, prompt.compiled_hash, prompt.status.value))


def log(category: str, theme: str, strategy: str, module: str, outcome: str, delta: float | None = None) -> None:
    with conn() as connection:
        connection.execute("INSERT INTO mutation_log VALUES(?,?,?,?,?,?,?,CURRENT_TIMESTAMP)", (new_id("mutation"), category, theme, strategy, module, delta, outcome))
...
def generate_variants(parent: Prompt, themes: list[FailureTheme], exemplars: list[dict], count: int = 6) -> list[Prompt]:
    ...
    while len(result) < count and attempts < count * 4:
        ...
        if digest in existing or _similar(child_modules, parent.modules):
            ...
            continue
        child = Prompt(prompt_id=new_id("prompt"), category=parent.category, modules=child_modules, gen_params=parent.gen_params,
                       parent_id=parent.prompt_id, lineage_depth=parent.lineage_depth + 1,
                       mutation_note=f"[{strategy}] {module}; target: {theme}", compiled_hash=digest)
        _persist(child)
