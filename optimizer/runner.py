...
def dataset_snapshot_id(category: str, split: str) -> str:
    with conn() as connection:
        rows = connection.execute(
            "SELECT case_id FROM benchmark_cases WHERE category=? AND split=? ORDER BY case_id",
            (category, split),
        ).fetchall()
...
def open_manifest(model_tag: str, category: str, split: str) -> RunManifest:
    compiler_sha = code_sha()
    manifest = RunManifest(
        run_id=new_id("run"), model_tag=model_tag,
        dataset_snapshot_id=dataset_snapshot_id(category, split),
    ...
    with conn() as connection:
        connection.execute(
            ...
            VALUES (?,?,?,?,?,?,?,?,?)""",
            (
                manifest.run_id, manifest.model_tag, manifest.judge_tag,
                manifest.rubric_version, manifest.dataset_snapshot_id,
                manifest.compiler_sha, manifest.evaluator_sha, split, category,
            ),
        )
    return manifest
...
def load_cases(category: str, split: str) -> list[BenchmarkCase]:
    with conn() as connection:
        rows = connection.execute(
            "SELECT * FROM benchmark_cases WHERE category=? AND split=? ORDER BY difficulty,case_id",
            (category, split),
        ).fetchall()
    return [
        BenchmarkCase(
            case_id=row["case_id"], category=row["category"], input=json.loads(row["input"]),
            expected=json.loads(row["expected"]) if row["expected"] else None,
            difficulty=row["difficulty"], split=row["split"], source=row["source"],
...
def load_prompt(prompt_id: str) -> Prompt:
    ...
    return Prompt(
        ...
        modules=PromptModules.model_validate_json(row["modules"]),
        gen_params=GenParams.model_validate_json(row["gen_params"]),
        parent_id=row["parent_id"], lineage_depth=row["lineage_depth"],
        mutation_note=row["mutation_note"], compiled_hash=row["compiled_hash"],
        status=row["status"],
    )
...
async def run_benchmark(backend, run_id: str, prompt: Prompt, cases: list[BenchmarkCase]) -> None:
    ...
    with conn() as connection:
            ...
            for row in connection.execute(
                "SELECT case_id FROM eval_results WHERE run_id=? AND prompt_id=?",
                (run_id, prompt.prompt_id),
            )
        }

    # Fallback evaluator for categories not in EVALUATORS — returns a neutral result.
    def _get_evaluators(cat: str):
        if cat in EVALUATORS:
            return EVALUATORS[cat]
        return ()
    ...
    async def evaluate(case: BenchmarkCase) -> None:
        async with limiter:
            ...
            try:
                # Timeout wraps only a single generation attempt, not the retry loop.
                output = await asyncio.wait_for(
                    backend.generate(rendered, case.input, case.expected, prompt.gen_params),
                    timeout=CASE_TIMEOUT_SECONDS,
                )
            except Exception as exc:
                output = f"ERROR: {type(exc).__name__}: {exc}"
            latency_ms = int((time.perf_counter() - start) * 1000)
            evaluator_fns = _get_evaluators(case.category)
            metrics = [metric(case, output).model_dump() for metric in evaluator_fns]
            with conn() as connection:
                connection.execute(
                    "INSERT OR IGNORE INTO eval_results VALUES (?,?,?,?,?,?)",
                    (run_id, prompt.prompt_id, case.case_id, output, dumps(metrics), latency_ms),
                )
...
def per_case_scores(run_id: str, prompt_id: str) -> dict[str, float]:
    with conn() as connection:
        rows = connection.execute(
            "SELECT case_id,metrics FROM eval_results WHERE run_id=? AND prompt_id=?",
            (run_id, prompt_id),
        ).fetchall()
    result: dict[str, float] = {}
    for row in rows:
        primary = next(
            (metric["score"] for metric in json.loads(row["metrics"]) if metric["name"].startswith("primary:")),
            None,
        )
...
def find_completed_run(prompt_id: str, model_tag: str, category: str, split: str) -> str | None:
    ...
    with conn() as connection:
        row = connection.execute(
            """SELECT r.run_id FROM run_manifests r JOIN eval_results e ON e.run_id=r.run_id
            ...
            (prompt_id, model_tag, category, split, snapshot, category, split),
        ).fetchone()
